  ...
  <div class="col-md-12">
    <div class="card">
      <div class="card-body">
        <div class="row">
          <div class="col-md-9">
            @include('components/search', ['id' => 'search', 'class' => 'filter'])
          </div>
          <div class="col-md-2">
            @include('components/row_per_page', ['id' => 'row_per_page', 'class' => 'filter'])
          </div>
          <div class="col-md-1">
            @include('components/reload', ['id' => 'btn-reload'])
          </div>
        </div>
        <div class="row mt-2">
          <div class="col-md-12">
            ...
            @include('components.pagination', ['id' => 'pagination'])
          </div>
        </div>
      </div>
    </div>
...
<script>
  ...
  function editData(id) {
    resetValidation();
    $('#id').val(id);
    $.get(`{{ url('lokasi_kerja') }}/${id}`).done(res => {
  ...
  function deleteData(id) {
    Swal.fire({
      ...
      if (res.value) {
        $.delete(`{{ url('lokasi_kerja') }}/${id}`).done(res => {
  ...
  $('#btn-save').click(function() {
    Swal.fire({
      ...
      if (res.value) {
        ...
        const promise = id == '' ? $.post("{{ url('lokasi_kerja') }}", data) : $.put(`{{ url('lokasi_kerja') }}/${id}`, data);
        promise.done(res => {
          swal(res.success ? 'success' : 'error', res.msg);
          if (res.success) {
            $('#lokasi-modal').modal('toggle');
